...
CMD ["sh", "-c", "if [ -z \"$BETTER_AUTH_SECRET\" ] || [ \"$BETTER_AUTH_SECRET\" = \"replace-with-a-secure-secret\" ]; then echo 'BETTER_AUTH_SECRET must be set to a non-placeholder value.' >&2; exit 1; fi; CI=true corepack pnpm install --frozen-lockfile && corepack pnpm db:generate && corepack pnpm --filter @mrsign/web dev --hostname 0.0.0.0"]
